            ...
            {loading ? (
                <ul className='flex flex-col'>
                    {Array.from({ length: TOP_COUNT }).map((_, index) => (
                        <li key={index} className='border-b border-hairline px-6 py-3.5 last:border-b-0'>
                            <div className='h-4 w-2/3 animate-pulse rounded bg-foreground/10' />
                        </li>
                    ))}
                </ul>
            ) : top.length === 0 ? (
                <p className='px-6 py-6 text-sm text-muted'>No domains indexed yet.</p>
            ) : (
                <ul className='flex flex-col'>
                    {top.map((entry, index) => (
                        <li key={entry.domain} className='flex items-center gap-3 border-b border-hairline px-6 py-3 last:border-b-0'>
